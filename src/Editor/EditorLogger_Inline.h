...
namespace paperback
{
	...
	void EditorLogger::Init(void)
	{
		...
		m_CoreLogger = std::make_shared<spdlog::logger>("Paperback Engine", Sinks.begin(), Sinks.end());
		m_CoreLogger->set_level(spdlog::level::trace);
		m_CoreLogger->flush_on(spdlog::level::trace);
		spdlog::register_logger(m_CoreLogger);

		m_ClientLogger = std::make_shared<spdlog::logger>("Editor", Sinks.begin(), Sinks.end());
