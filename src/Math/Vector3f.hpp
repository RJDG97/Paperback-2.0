...
namespace paperback
{
	...
	// the old velocity cap is (vec_sq > float_sq) -> vec.normalized() * float
	//   -- think of the problem with a box, not a sphere like the example above
	//  ----- refer to A4 paper why this is done this way, for debug
	...
	// can use float also, but for more flexibility, using vector
